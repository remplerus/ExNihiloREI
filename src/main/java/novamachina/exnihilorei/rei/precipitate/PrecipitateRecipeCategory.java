...
public class PrecipitateRecipeCategory implements DisplayCategory<SimpleBasicDisplay> {
  ...
  public List<Widget> setupDisplay(SimpleBasicDisplay display, Rectangle bounds) {
    ...
    widgets.add(Widgets.createSlot(new Point(startPoint.x + 48, startPoint.y + 37))
            .entries(display.getInputEntries().get(1)).markInput());
    widgets.add(Widgets.createSlot(new Point(startPoint.x + 75, startPoint.y + 10))
            .entries(display.getInputEntries().get(0)).markInput());

    widgets.add(Widgets.createSlot(new Point(startPoint.x + 102, startPoint.y + 37))
